...
class US_Unemployment_Rate:
    def __init__(self, api_key):
        """
        Initialize the USUnemploymentRate class with the API key and default parameters.
        """
        self.api_key = api_key
        self.endpoint = "https://api.stlouisfed.org/fred/series/observations"
        self.series_id = "UNRATE"  # Unemployment Rate

    def fetch_data(self):
        """
        Fetch Unemployment Rate data from the FRED API and return it as a DataFrame.
        """
        params = {
    ...
    def process_data(self, df):
        """
        Process the Unemployment Rate data by calculating MoM and YoY changes.
        """
        df = df.copy()
        df.sort_values(by="Data", inplace=True)
        df["MoM (%)"] = df["Valor"].diff()
        df["YoY (%)"] = df["Valor"].diff(periods=12)
        return df
    ...
    def run(self, pickle_path_raw, pickle_path_treated, excel_path):
        """
        Execute the full pipeline: fetch, process, and save Unemployment Rate data.
        """
        # Fetch raw data
