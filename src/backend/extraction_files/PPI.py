...
class PPI:
    def __init__(self, api_key):
        """
        Initialize the PPI class with the API key and default parameters.
        """
        self.api_key = api_key
        self.endpoint = "https://api.stlouisfed.org/fred/series/observations"
        self.series_id = "PPIACO"  # Producer Price Index for All Commodities

    def fetch_data(self):
        """
        Fetch PPI data from the FRED API and return it as a DataFrame.
        """
        params = {
    ...
    def process_data(self, df):
        """
        Process the PPI data by calculating MoM and YoY changes.
        """
        df = df.copy()
    ...
    def run(self, pickle_path_raw, pickle_path_treated, excel_path):
        """
        Execute the full pipeline: fetch, process, and save PPI data.
        """
        # Fetch raw data
